...
\begin{definition}
	...
	\[RS_{\mathbb{F}_q}(S, k) = \big\{(f(\alpha_1), \dots, f(\alpha_n)) \mid f \in \mathbb{F}_q^k[X], f(X) = \sum_i^{k-1}(X^i f_i(X)) \big\}.\]
\end{definition}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\[f(X) = \sum_i^{k-1}(X^i f_i(X)) = \sum_i^{k-1}(X^i 0) = 0,\]
		...
		y se verifica
		\begin{align*}
			(f_\textbf{x} + f_\textbf{y})(X) = f_\textbf{x}(X) + f_\textbf{y}(X) &= \sum_i^{k-1}(X^i {f_\textbf{x}}_i(X)) + \sum_i^{k-1}(X^i {f_\textbf{y}}_i(X))\\
			&= \sum_i^{k-1}(X^i({f_\textbf{x}}_i + {f_\textbf{y}}_i)(X))\\
			&= \sum_i^{k-1}(X^i(f_\textbf{x} + f_\textbf{y})_i(X)).
		\end{align*}
		...
		y se verifica
		\[(\lambda f)(X) = \lambda f(X) = \lambda \sum_i^{k-1}(X^i f_i(X)) = \sum_i^{k-1}(X^i \lambda f_i(X)) = \sum_i^{k-1}(X^i (\lambda f_i)(X)).\]
	\end{enumerate}
\end{proof}
