...
\paragraph{Generación de claves}
\begin{enumerate}
	\item Primero, se escogen dos números primos $p$ y $q$. Si bien el algoritmo funciona con cualesquiera primos, se recomienda el uso de valores elevados. Estos dos primos son la \textit{información adicional} que facilitará obtener la preimagen de una función irreversible con trampa.

	\item Una vez escogidos los primos, se calcula $n = pq$, valor al que se denomina \textbf{módulo}, y $\phi(n) = (p - 1)(q - 1)$.

	\item Después, se selecciona un número $e$ tal que $1 < e < \phi$ y el menor común múltiplo entre $e$ y $phi$ es $1$.

	\item Finalmente, se utiliza el algoritmo euclideano extendido para calcular un valor $d$ tal que $ed \equiv 1 \mod \phi$. 
\end{enumerate}

Así, se genera un par $((e, n), d)$, donde $(e, n)$ es la clave pública y $d$ es la clave privada.

\paragraph{Cifrado}
\begin{enumerate}
	\item Se computa $m \in [0, n) \cap \integers$ tal que el mensaje queda representado en $m$.
	
	\item Se calcula $c = m^e \mod n$.
\end{enumerate}

El valor $c$ es el texto cifrado.

\paragraph{Descifrado}
\begin{enumerate}
	\item Se computa $m = c^d \mod n$.
\end{enumerate}

El valor $m$ representa al texto plano.
...
\begin{proof}
	El texto cifrado se define por $c = m^e \mod n$. El texto descifrado se da por $m' = c^d \mod n$. Por sustitución, se obtiene $m' = (m^e)^d \mod n = m^{ed} \mod n = m \mod n$, ya que $ed \equiv 1 \mod n$ según el algoritmo de generación de claves.
\end{proof}
